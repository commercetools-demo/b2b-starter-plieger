  ...
  const columns = [
    {
      ...
      render: (row: any) => {
        ...
        return (
          <a href={`/dashboard/purchase-lists/${row.id}`} className="text-primary font-medium hover:underline">
            {name}
          </a>
        );
      },
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6">
        <Table columns={columns} data={lists} loading={loading} emptyMessage="No purchase lists." onRowClick={(row: any) => router.push(`/dashboard/purchase-lists/${row.id}`)} />
      </div>
